...
Улучшение считается устойчивым, если на > 60% пилотных миссий TtFC с ML даёт выигрыш по сравнению с ручным TtFC на 24%.
...
Критерий успеха Дельта T_review >= 0.5:
...
Улучшение считается устойчивым, если на > 50% пилотных миссий за 10 минут облета (150 фото на 3 га), автоматическая обработка занимает 2 минуты, затем подтверждение в панели верификации 3 минуты (против 10-минутного ручного мониторинга), при этом улучшение не достигается ценой пропуска целей (см. п. 3)
...
4. Контроль ложных тревог - система ограничивает количество ложных алертов, чтобы не повышать нагрузку на оператора в части отвлечения на проверку подозрительных сигналов. Количество ложных алертов (человек обнаруживается 1 секунду подряд) не должно превышать 7 в час в дежурном режиме, когда людей чаще всего нет, и 12 в в час режиме активного поиска, когда важнее не пропустить.
...
MVP - рабочий прототип для демонстрации: система принимает видео/поток, детектит person, показывает боксы/алерты и базовый контекст. На этапе MVP эффект по бизнес-метрикам измеряется вручную: оператор смотрит видео без подсказок и по таймкоду фиксирует момент первого подтверждения цели и оценивает время активной проверки (вручную в таблице).

Пилот - проверка бизнес-эффекта в операторском процессе по регламенту: один и тот же материал проходит два сопоставимых прогона (ручной режим vs режим с ML), система автоматически логирует алерты и действия оператора (подтверждения/отклонения), формирует отчёт и рассчитывает метрики из п. 1.1. По отчету принимается решение о дальнейших итерациях.
