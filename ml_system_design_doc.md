...
В этот интервал попал только алерт `12.3 c`, так как  `8.5 c` <  `9.0 c`, а `41.0 c` относится к другому эпизоду, тогда:

```math
TtFC = t_{\text{confirm}} - t_{\text{start}} = t_{\text{confirm}} - s_{1} = 12.3 - 10.0 = 2.3\ \text{с}
```

То есть система среагировала через 2,3 секунды после начала появления человека в кадре.
...
```math
\langle class_{id}, x, y, w, h\rangle
```
