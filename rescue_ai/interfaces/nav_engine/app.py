...
Внутри — ровно тот же ``rescue_ai.infrastructure.navigation.engine.NavigationEngine``,
...
from rescue_ai.interfaces.nav_engine.schemas import (
    ResetRequest,
    ResetResponse,
    StepRequest,
    StepResponse,
...
class _SessionRegistry:
    ...
    def create(self, mission_id: str, fps: float | None) -> str:
        ...
        with self._lock:
            self._sessions[session_id] = _Session(engine=engine, mission_id=mission_id)
        return session_id
...
def build_app(*, registry: _SessionRegistry | None = None) -> FastAPI:
    ...
    def create_session(payload: ResetRequest) -> ResetResponse:
        session_id = sessions.create(mission_id=payload.mission_id, fps=payload.fps)
        nav_mode = _coerce_nav_mode(payload.nav_mode)
        sessions.get(session_id).engine.reset(nav_mode=nav_mode, fps=payload.fps)
        logger.info(
