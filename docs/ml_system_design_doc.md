...
- Сервисы `api_gateway`, `detection_service`, `flight_adapter`, UI оператора и вспомогательные сервисы запускаются в Kubernetes на наземной станции
- Кадры после приёма попадают в очередь обработки, а основной и резервный сервисы инференса работают как потребители этой очереди
- Рабочая версия модели предварительно загружается из S3 и хранится на узле инференса; во время активной миссии сервис инференса использует локальную копию модели
...
В продакшн-интеграции система работает в связке **ArduPilot + companion computer (RaspberryPi/Orin) + MAVLink:**
- Companion computer получает по MAVLink телеметрию и статусы автопилота,
- ML-сервис использует их только для логики миссии (приём потока, инференс, постобработку, алерты, журнал миссии и восстановление после сбоев вычислительной части) и может отправлять лишь высокоуровневые команды безопасного режима: `RTL (возвращение в точку старта)` или `Land (посадка на текущем месте`, через отдельный `flight_adapter`
...
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **Разделение критического и фоновых сервисов**              | В критический контур миссии входят приём потока, очередь кадров, инференс, постобработка алертов и UI оператора. PostgreSQL и генерация отчёта работают асинхронно и не блокируют онлайн-обработку<br><br>`api_gateway`, `detection_service` и `flight_adapter` развёрнуты как отдельные сервисы в **Kubernetes**; запись в **PostgreSQL**и выгрузка в **S3-compatible storage** выполняются отдельными writer-процессами | Отказ БД, S3 или сервиса отчётов не останавливает миссию и не прекращает выдачу алертов оператору  |
| **Журнал миссии**                                           | Для каждого кадра и события фиксируются `mission_id`, `frame_id`, `timestamp`, статус обработки, версия модели и текущий статус миссии. После рестарта сервисы читают последнее подтверждённое состояние из журнала и продолжают работу с него<br><br>                                                                                                                                                                    | При падении сервиса миссия не начинается заново и не теряет уже обработанные данные                |
| **Буферизация потока через очередь кадров**                 | Кадры после приёма не сразу идут в инференс, а попадают в ограниченную очередь. Если инференс временно замедлился, очередь сглаживает кратковременные пики нагрузки. Используем RabbitMQ quorum queue `frames.infer`; в очередь кладутся не сами изображения, а `mission_id`, `frame_id`, `timestamp`, путь к кадру                                                                                                       | Кратковременные просадки производительности не приводят к мгновенному срыву миссии                 |
| **Ограничение очереди**                                     | Если очередь растёт выше допустимого порога (20 кадров), на бэкенде включается `SAFE_MODE`: снижается частоту обработки, уменьшается разрешение инференса и перестают накапливаться лишние кадры                                                                                                                                                                                                                          | Система продолжает работать в упрощённом, но управляемом режиме                                    |
| **Проверка живости сервисов**                               | Для сервисов приёма потока, инференса, постобработки и UI задаются health-check’и: отвечает ли процесс, готов ли он принимать нагрузку, появляются ли новые результаты. Kubernetes liveness/readiness/startup probes на эндпоинты `/health`, `/ready`, `/startup`                                                                                                                                                         | Система понимает, что сервис реально работает или уже вышел из строя                               |
| **Контроль фактической активности компонентов**             | Метрики `no_frame_duration`, `no_inference_result_duration`, `telemetry_gap_sec` экспортируются в Prometheus; правила в Alertmanager фиксируют зависания                                                                                                                                                                                                                                                                  | Ловятся случаи, когда процесс формально жив, но фактически перестал выполнять свою функцию         |
| **Автоперезапуск сервисов**                                 | Сервисы запускаются в Kubernetes, который при сбое сам перезапускает их и возвращает в работу после проверки готовности                                                                                                                                                                                                                                                                                                   | Кратковременные программные сбои устраняются без ручного вмешательства оператора                   |
| **Резервный сервис инференса**                              | В Kubernetes постоянно запущены два экземпляра сервиса инференса. Очередь кадров реализована через RabbitMQ, и в каждый момент сообщения получает только один активный consumer. Если основной экземпляр падает, RabbitMQ переключает доставку на резервный, а Kubernetes автоматически перезапускает отказавший экземпляр и возвращает его в резерв                                                                      | Отказ одного сервиса инференса не приводит к полной остановке детекции                             |
| **Локальный кэш модели на inference-узле**                  | Актуальная модель скачивается из **S3-compatible storage** в каталог `/models/current` и используется локально активным и резервным pod’ами                                                                                                                                                                                                                                                                               | Отказ S3 во время активной миссии не останавливает инференс, если локальная копия уже есть         |
| **Локальное сохранение данных при отказе внешних хранилищ** | Если PostgreSQL или S3 недоступны, метаданные, ключевые кадры и события миссии временно сохраняются локально и ставятся в очередь на последующую синхронизацию.                                                                                                                                                                                                                                                           | Миссия продолжается даже при отказе внешней инфраструктуры                                         |
| **Идемпотентная запись результатов**                        | В PostgreSQL задаются `UNIQUE(mission_id, frame_id)` и `UNIQUE(mission_id, alert_id)`; повторная обработка делает upsert, а не новую запись                                                                                                                                                                                                                                                                               | После рестартов и повторной обработки система не дублирует события                                 |
| **Независимый контур flight safety**                        | На борту работает `flight_adapter`, который получает телеметрию от **ArduPilot** по **MAVLink** через companion computer и может отправлять только высокоуровневые команды `RTL` / `Land`                                                                                                                                                                                                                                 | Критические полётные сценарии не зависят от состояния ML-модели, UI или внешних хранилищ           |
| **Преднастроенные failsafe-сценарии на борту**              | В **ArduPilot** заранее настраиваются `GCS failsafe`, `Battery failsafe`, `RTL` profile и допустимые сценарии `Land` / `SmartRTL`                                                                                                                                                                                                                                                                                         | Даже если сервер детекции недоступен, БПЛА всё равно может завершить полёт по безопасному сценарию |
| **Явные статусы деградации и аварии**                       | Миссия всегда находится в одном из понятных состояний: `RUNNING`, `DEGRADED`, `SAFE_MODE`, `LINK_LOSS`, `BATTERY_ABORT`, `EMERGENCY_TERMINATE`.                                                                                                                                                                                                                                                                           | Оператор видит, что именно случилось и в каком режиме сейчас работает система                      |

#### 4.3.3. Сценарии отказов и действия системы

| Сценарий                                                                                                | Как обнаруживаем                                                      | Что делает ML-система                                                                                                                                                                                  | Что делает автопилот                                                                          | Результат                                                                                          |
| ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **Погодные условия вышли за допустимые**: сильный ветер, дождь, снег, туман, резкое ухудшение видимости | статусы автопилота, резкое ухудшение наблюдаемости и качества потока  | переводит миссию в `WEATHER_ABORT`, завершает обработку уже принятых кадров, сохраняет журнал миссии и передаёт команду в flight-safety контур на безопасное завершение полёта                         | при наличии связи выполняется заранее выбранный сценарий: `RTL`или `Land`                     | миссия завершается контролируемо, данные не теряются                                               |
| **Потеря связи с дроном (lost link)**                                                                   | нет heartbeat/телеметрии от БПЛА дольше 5 секунд                      | фиксирует `LINK_LOSS`, сохраняет последнее известное состояние миссии, замораживает основной рабочий контур и перестаёт ожидать новые кадры, после чего ждёт результата от бортового failsafe          | сам запускает преднастроенный lost-link failsafe                                              | если связь восстановилась — миссия может продолжиться; если нет — БПЛА завершает полёт по failsafe |
| **Критический отказ БПЛА**: аппарат сбит, разрушен, физически потерян                                   | резкий обрыв потока и телеметрии, отсутствие ответа flight controller | переводит миссию в `EMERGENCY_TERMINATE`, очередь закрывается, сохраняются последний кадр, последняя телеметрия, версия модели и журнал алертов                                                        | дополнительного действия уже нет: аппарат считается потерянным или аварийно завершившим полёт | миссия аварийно завершена, система сохраняет данные для разбора инцидента                          |
| **Критически низкий заряд батареи**                                                                     | warning/critical battery от автопилота                                | переводит миссию в `BATTERY_ABORT`, прекращает выдачу новых поисковых задач, сохраняет состояние миссии, инициирует безопасное завершение миссии через flight-safety контур                            | выполняет low-battery failsafe: `RTL` или `Land`                                              | поиск прекращается безопасно, снижается риск потери БПЛА                                           |
| **Сервис инференса упал или завис**                                                                     | 3 подряд неуспешных health-check, нет новых результатов инференса     | Kubernetes выводит активный инстанс из рабочего пула и переключает очередь на резервный `inference-B`; упавший `inference-A`перезапускается, загружает модель, проходит warm-up и становится резервной | не участвует                                                                                  | миссия продолжается после короткой паузы                                                           |
| **Поток с БПЛА пропал или RTSP/UDP-сессия зависла**                                                     | `no_frame_duration > 2 с` или повторяются одинаковые кадры/таймкоды   | Watchdog переводит поток в `DEGRADED`, выполняет реконнект с бэкоффом `0.5 c, 1 c, 2 c, 4 c, 8 c`, перезапускает приёмник и декодер                                                                    | не участвует, пока связь с БПЛА не потеряна полностью                                         | при восстановлении потока миссия продолжается без ручного рестарта                                 |
| **Пайплайн перегружен**: растёт очередь, latency выходит за допустимый предел                           | рост `queue_len`, `latency_p95`, перегрузка CPU/RAM                   | включается `SAFE_MODE`: снижается частота обработки, уменьшается разрешение инференса, ограничивается длина очереди                                                                                    | не участвует                                                                                  | миссия продолжается в деградированном режиме вместо полного отказа                                 |
...
| **Модель не загрузилась / артефакт повреждён**                                                          | ошибка инициализации, несовпадение контрольной суммы файла            | сервис переключается на последнюю валидированную модель, фиксирует `MODEL_FALLBACK` и продолжает работу                                                                                                | не участвует                                                                                  | миссия продолжается с возможной деградацией качества, но без полной остановки                      |
| **PostgreSQL недоступен**                                                                               | ошибки записи, таймауты metadata-writer                               | метаданные временно сохраняются в локальный журнал миссии и очередь на синхронизацию                                                                                                                   | не участвует                                                                                  | основной контур не прерывается, синхронизация выполняется позже                                    |
| **S3-хранилище недоступно**                                                                             | ошибки upload/timeout                                                 | ключевые кадры и отчёты временно сохраняются локально, включается очередь `to_sync_s3`                                                                                                                 | не участвует                                                                                  | алертинг продолжается, откладывается только выгрузка артефактов                                    |
...
#### 4.3.4. Безопасное завершение миссии
...
4. Если связь с автопилотом ещё есть, `flight_adapter` **отправляет высокоуровневую команду** безопасного режима во flight-safety контур на один из заранее определённых сценариев:
...
#### 4.3.5. Наблюдаемость и эксплуатационные сигналы
...
- **Инференс:** `inference_errors_total`, `health_failures_total`, `failover_events_total`, `model_fallback_events_total`.
    
- **Производительность:** `queue_len`, `latency_p50`, `latency_p95`, `fps_infer`, `safe_mode_activations_total`.
    
- **Алертинг:** `alerts_total`, `alerts_per_min`, доля подтверждённых и отклонённых алертов, число включений строгого профиля.
    
- **Хранилища:** длина очередей `to_sync_db` и `to_sync_s3`, количество невыгруженных ключевых кадров.
...
- **Инфраструктура:** затраты на Kubernetes-контур, мониторинг и поддержание резервного экземпляра инференса.
- **Хранение данных:** S3 для ключевых кадров/метаданных/отчетов; политики хранения и удаления. Объем хранения данных на миссию оценивается через средний размер кадра с алертом $S_{kf}$ (около 400 KB на JPG), число алертов в миссии $N_{alert}$ и объем метаданных $V_{\text{meta}}$ (JSON/CSV), которые обычно значительно легче ключевых кадров:
$$
