...
2. **Полезность подсказок на уровне эпизодов (Recall_event)**  
   Эпизод `k` считается найденным, если существует алерт `t \in A`, попавший в интервал `[s_k - \tau, e_k + \tau]`, и:
...
Таким образом, успешный пилот — это пилот, в котором на `>50%` миссий достигается измеримое улучшение по `\Delta TtFC` при одновременном выполнении ограничений по `Recall_{\text{event}}` и `FP/\min`, без критичных задержек и без опоры на облачную инфраструктуру.
