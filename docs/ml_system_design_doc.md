...
Ниже приведена целевая архитектура решения, согласованная с текущим MVP: видеопоток поступает в локальный контур, выполняется детекция, формируются алерты и сохраняются артефакты миссии (логи/отчёт).

```mermaid
%%{init: {"flowchart": {"curve": "linear"}, "theme": "base"} }%%
flowchart LR

CAM["DJI Mavic 3<br/>камера (видеопоток)"]
EDGE["Edge-узел: Raspberry Pi 4B<br/>прием и буферизация"]
SRV["Сервис инференса<br/>(FastAPI)<br/>декодирование → preprocess → YOLO"]
POST["Постобработка<br/>детекции → алерты<br/>(conf_thr, W, k, cooldown, tau_gap_end)"]
UI["UI оператора / веб-панель<br/>кадр + bbox + алерты"]
LOG["Логи миссии<br/>алерты / таймкоды / параметры"]
REP["Генератор отчета<br/>TtFC, Recall_event, FP/min"]

CAM -->|передача видео| EDGE
EDGE -->|RTSP / UDP| SRV
SRV --> POST
POST --> UI
POST --> LOG
...
class CAM,EDGE,SRV,POST stageBlue;
class UI,REP stageGreen;
class LOG stageOrange;
```

**Architecture schema:** [Открыть architecture schema](./architecture_schema.png)

**Параметры алертинга (обозначения в формулах и конфиге)**

| Параметр | Смысл | Единицы / тип |
|---|---|---|
| `conf_thr` | Порог confidence для детекции `Person` | `0..1` |
| `W` | Окно агрегации детекций в алерт | секунды |
| `k` | Минимум подтверждающих детекций в окне `W` | целое |
| `cooldown` | Пауза перед повторной генерацией алерта | секунды |
| `tau_gap_end` (`τ_gap_end`) | Допустимый разрыв для завершения/склейки эпизода | секунды |

**Примечание про контуры исполнения**
- В MVP допускается, что **инференс выполняется на наземной станции** (но поток поступает с RaspberryPi), т.к. это упрощает отладку и демонстрацию.
- В целевом production‑контуре инференс может быть перенесён на edge‑узел (Raspberry Pi/Orin) с передачей на наземную станцию только алертов/ключевых кадров.
