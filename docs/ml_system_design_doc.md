...
Здесь описывается целевая архитектура и требования к эксплуатационному контуру. Для MVP‑пилота (офлайн/локальный контур без облака) допускается упрощённая реализация.  
...
- Данные пилота и другие артефакты выносятся во внешнее хранилище (PostgreSQL).
...
- ограничение доступа к пилоту по ролям,
- ограничение срока хранения исходного потока (по необходимости хранить только алерты/таймкоды/ключевые кадры),
- при публикации/передаче — обезличивание (например, блюр лица) и соблюдение требований законодательства (в т.ч. ФЗ-152).

### 4.6. Издержки

**Production‑контур:** основные статьи затрат:
- вычислительный модуль на борт/наземную станцию (периферийное железо),
