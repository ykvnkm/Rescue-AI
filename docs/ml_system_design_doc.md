...
> Раздел 4 описывает целевую архитектуру и требования к эксплуатационному контуру. Для MVP‑пилота (офлайн/локальный контур без облака) допускается упрощённая реализация.  
...
**MVP (локальный контур без облака):**
- Edge‑узел (Raspberry Pi 4B) принимает поток и передаёт его в сервис (локальная сеть/радиоканал).
- Сервис инференса и UI запускаются на ноутбуке/НСУ (Docker/Docker Compose допустимы).
- Данные пилота (логи/отчёты) хранятся локально; выгрузка во внешние хранилища — опционально.

**Масштабирование (если несколько БПЛА/потоков):**
- Каждый поток обрабатывается отдельным инстансом пайплайна (изолированные конфиги, отдельные очереди/топики).
- Для буферизации и разгрузки пиков возможно использование брокера сообщений (например, Redis Streams/Kafka) и горизонтальное масштабирование инференса.

### 4.3. Требования к работе системы

Требования формулируются на уровне пользовательского сценария “подсказки в ходе миссии”:
- **Устойчивость**: непрерывная работа в течение миссии; автоматическое восстановление при кратковременных обрывах потока.
- **Задержка**: контролируется `p50/p95 latency` (измеряется на этапе 2.3.4 и в пилоте). В MVP фиксируется факт “без критичных задержек”, в production задаётся SLA на основе результатов пилота.
- **Пропускная способность**: обеспечивать обработку потока в режиме, достаточном для формирования алертов (окно `W` и параметры `k`, `cooldown`, `tau_gap_end` должны быть корректно применимы на фактической частоте кадров).
- **Наблюдаемость**: логирование ключевых событий (получение кадра, инференс, алерт, ошибки декодирования/потока).

### 4.4. Безопасность системы

Потенциальные уязвимости:
- несанкционированный доступ к API/панели,
- подмена/инъекция видеопотока,
- перегрузка сервиса запросами (DoS) в случае сетевой доступности.

Меры (минимум для production):
- запуск сервисов в закрытом контуре сети (без публичного доступа),
- аутентификация для UI/API, ограничение источников потока (allow‑list),
- rate limiting на API, отдельные ключи/токены для компонент,
- журналирование доступа и ошибок.

### 4.5. Безопасность данных

Так как видеопоток потенциально содержит персональные данные:
- хранение данных пилота локально, ограничение доступа по ролям,
- ограничение срока хранения сырого видео (по необходимости хранить только алерты/таймкоды/ключевые кадры),
- при публикации/передаче — обезличивание (например, блюр лица) и соблюдение требований законодательства (в т.ч. GDPR при соответствующем контуре использования).

### 4.6. Издержки

**MVP‑контур:** издержки минимальны (локальная инфраструктура, существующие устройства). Основные затраты — время команды на обучение/оценку моделей и интеграцию пайплайна.

**Production‑контур:** основные статьи затрат:
- compute‑модуль на борт/наземную станцию (edge‑железо),
- сопровождение модели (переоценка на новых миссиях, переобучение),
- поддержка инфраструктуры (мониторинг, хранение данных, обновления).

### 4.7. Integration points

Основные точки интеграции:
- **Вход потока:** RTSP/UDP поток (H.264/H.265) с edge‑узла.
- **API сервиса:** эндпоинты вида `/health`, `/infer` (если кадры приходят как запросы), `/alerts`, `/report/{mission_id}`, `/config`.
- **Выход:** алерты (JSON/CSV), отчёты по миссиям (CSV/JSON/HTML), опционально — ключевые кадры.

### 4.8. Риски

- **Потеря/обрыв потока:** буферизация на edge‑узле, реконнект, запись “сырых” артефактов для последующего разбора.
- **Срыв по latency на edge‑железе:** профилирование, упрощение пайплайна, переход на более лёгкую модель/режим, оптимизации.
- **Дрифт условий (сезон/освещение/высота):** регулярная переоценка на новых миссиях, обновление датасета, повторная калибровка порогов.
- **Рост FP/min при улучшении recall:** управление компромиссом через конфиг алертинга, калибровка и оптимизатор (этап 2.3.6).
