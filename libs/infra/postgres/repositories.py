...
from libs.core.application.mission_metrics import build_gt_episodes
...
class PostgresAlertRepository(AlertRepository):
    ...
    def add(self, alert: Alert) -> None:
        primary = alert.evidence.primary_detection
        with self._db.connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    ...
                    (
                        ...
                        alert.ts_sec,
                        alert.image_uri,
                        alert.evidence.people_detected,
                        ...
                        json.dumps(
                            [
                                ...
                                for detection in alert.evidence.detections
                            ]
                        ),
                        alert.lifecycle.status,
                        alert.lifecycle.reviewed_by,
    ...
    def update_status(
        ...
        if target_status not in self.allowed_target_statuses:
...
def _serialize_detection(detection: DetectionData) -> dict[str, object]:
    return {
...
def _detection_from_payload(payload: dict[str, Any]) -> DetectionData:
...
def _coerce_bbox(value: Any) -> tuple[float, float, float, float]:
    ...
    return tuple(float(item) for item in payload)  # type: ignore[return-value]
