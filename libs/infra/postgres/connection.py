...
def resolve_postgres_dsn(
    ...
    port = _clean(values.get("APP_POSTGRES_PORT")) or "5432"
    database = _clean(values.get("APP_POSTGRES_DB"))
    user = _clean(values.get("APP_POSTGRES_USER"))
    password = values.get("APP_POSTGRES_PASSWORD")
    has_component_values = any(
        value is not None for value in (host, database, user, _clean(password))
    )
    ...
    missing = [
        ...
        for name, value in (
            ...
            ("APP_POSTGRES_PASSWORD", _clean(password)),
        )
        if value is None
    ]
    if missing:
        ...
        raise ValueError(
            "Incomplete Postgres settings. Set APP_POSTGRES_DSN or all of: "
            "APP_POSTGRES_HOST, APP_POSTGRES_PORT, APP_POSTGRES_DB, "
            f"APP_POSTGRES_USER, APP_POSTGRES_PASSWORD. Missing: {missing_list}"
        )
